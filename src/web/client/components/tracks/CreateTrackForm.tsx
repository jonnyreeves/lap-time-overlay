...
import type { CreateTrackFormCreateTrackMutation } from "../../__generated__/CreateTrackFormCreateTrackMutation.graphql.js";
...
const CreateTrackMutation = graphql`
  mutation CreateTrackFormCreateTrackMutation($input: CreateTrackInput!) {
...
export function CreateTrackForm({ onCancel, onSuccess }: CreateTrackFormProps) {
  ...
  const [commit, isInFlight] = useMutation<CreateTrackFormCreateTrackMutation>(CreateTrackMutation);
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    commit({
      ...
      onCompleted: (response) => {
        const newTrackId = response.createTrack?.track?.id;
        if (!newTrackId) {
          setFormError("Track created, but the response was missing its ID.");
          return;
        }
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div css={buttonGroupStyles}>
        {onCancel ? (
          <button type="button" css={secondaryButtonStyles} onClick={handleCancel} disabled={isInFlight}>
            Cancel
          </button>
        ) : null}
        <button
