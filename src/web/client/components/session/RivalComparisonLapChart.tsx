...
function RivalComparisonLapChartSvg({
  comparisons,
  rivalName,
  rivalClassification,
  width,
  ...
  const hoveredSelfY = hoveredLap ? projectY(hoveredLap.selfLap) : null;
  const hoveredRivalY = hoveredLap ? projectY(hoveredLap.rivalLap) : null;
  ...
  const tooltipY =
    hoveredSelfY != null && hoveredRivalY != null
      ? resolveChartTooltipY([hoveredSelfY, hoveredRivalY], tooltipHeight, yPad, plotBottomY)
      : yPad;
  ...
  return (
    ...
    >
      ...
      {hoveredLap && hoveredX != null && hoveredSelfY != null && hoveredRivalY != null ? (
        <>
          <line
            x1={hoveredX}
            x2={hoveredX}
            ...
            strokeDasharray="4 4"
          />
          <circle cx={hoveredX} cy={hoveredSelfY} r={5.5} fill="#4f46e5" stroke="#ffffff" strokeWidth={2} />
          <circle cx={hoveredX} cy={hoveredRivalY} r={5.5} fill="#f97316" stroke="#ffffff" strokeWidth={2} />
          <g transform={`translate(${tooltipX ?? 12}, ${tooltipY})`}>
            ...
            <text x={12} y={34} fontSize="10" fill="#c7d2fe">
              You {formatLap(hoveredLap.selfLap)}
            </text>
            <text x={12} y={48} fontSize="10" fill="#fed7aa">
              {rivalName} {rivalClassification} {formatLap(hoveredLap.rivalLap)}
            </text>
          </g>
        </>
      ) : null}
...
export function RivalComparisonLapChart({ comparisons, rivalName, rivalClassification }: Props) {
  const [isExpanded, setIsExpanded] = useState(false);
  const hasCompressedOutliers = createCompressedLapTimeScale(
    comparisons.flatMap((lap) => [lap.selfLap, lap.rivalLap]),
    0,
    100
  ...
  return (
    <>
      <div css={chartCardStyles}>
        ...
        {comparisons.length > 0 ? (
          <RivalComparisonLapChartSvg
            comparisons={comparisons}
            rivalName={rivalName}
            rivalClassification={rivalClassification}
            width={720}
            height={284}
          />
        ) : (
      ...
      <Modal
        ...
        title="Rival Comparison Chart"
        maxWidth="1200px"
      >
        <div css={modalBodyStyles}>
          <p css={modalHintStyles}>Compare your pace lap by lap against the selected rival with more space for axes and hover detail.</p>
          {hasCompressedOutliers ? (
